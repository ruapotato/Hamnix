...
build_adder_user hamwrite             # scene-file DE HamWrite word processor: the office-suite flagship — File/Edit/Format menu bar + formatting toolbar, pixel word-wrap + scrolling, bold/italic/underline/heading, four text sizes, left/centre/right paragraph alignment, selection+clipboard, New/Open/Save/Save As of a HAMWRITE1 document container. PRE-INSTALLED first-class DE app (pkg hamnix-hamwrite, launcher etc/hamde/apps/hamwrite.desktop; lib/hamwritecore, dual-target; host gate: scripts/test_hamwrite_host.sh)
build_adder_user hamsheet             # scene-file DE HamSheet spreadsheet: the office suite's 2nd app — scrollable A/B/C… grid, cells hold numbers/text/=formulas with a recalculating engine (cell refs, + - * /, parens, SUM/AVG/MIN/MAX/COUNT over A1:A5), save/load a HAMSHEET1 document container that round-trips formulas. PRE-INSTALLED first-class DE app (pkg hamnix-hamsheet, launcher etc/hamde/apps/hamsheet.desktop; lib/hamsheetcore, dual-target; host gate: scripts/test_hamsheet_host.sh)
...
build_adder_user hamslides            # scene-file DE HamSlides presentation app: the office suite's 3rd app — a deck of slides (title + bullet list), an EDIT view with a thumbnail rail + large current slide and a full-window PRESENT view, save/load a HAMSLIDES1 document container that round-trips the deck. PRE-INSTALLED first-class DE app (pkg hamnix-hamslides, launcher etc/hamde/apps/hamslides.desktop; lib/hamslidescore, dual-target; host gate: scripts/test_hamslides_host.sh)
