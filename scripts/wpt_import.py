...
RUN BOTH IMPORTERS TOGETHER. This one owns the testharness.js lane; its sibling
scripts/wpt_reftest_import.py owns the REFTEST lane (pixel comparison of a test
against its <link rel="match"> reference). They share tests/wpt/tests/ and THIS
importer rebuilds that directory from scratch, so running it alone deletes the
vendored reftests. Both must be at the same PINNED_SHA.
