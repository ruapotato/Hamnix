...
installer_img_or_verdict "$INSTALLER_IMG" "[hambrowse_visual_ondevice]"
...
[ -s "$AFTER_PPM" ] || { echo "[hbvis] RESULT: INCONCLUSIVE (empty AFTER screendump — no scanout captured)"; exit 125; }
...
case "$rc" in
    ...
    2) echo "[hbvis] RESULT: INCONCLUSIVE"; exit 125 ;;
